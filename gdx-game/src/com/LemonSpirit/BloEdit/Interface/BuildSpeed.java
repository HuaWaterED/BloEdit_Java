...
public class BuildSpeed {
    Image back;
    Table SpeedListButton;
    ArrayList<squareEvent> list;
    Actor checked;
    WorldBuild build;
    public BuildSpeed(final WorldBuild build){
        this.build=build;
        list=GetData.SaveData.squareData.get(GetData.NowBlock-1).judgeLineData[GetData.NowPlane-1].speed;
        ...
        Label Title=new Label("SPEED LIST   控制此Line的流速",new Label.LabelStyle(build.Bit,build.Bit.getColor()));
        ...
        SpeedListButton=new Table();
        SpeedListButton.top();
        ScrollPane pane=new ScrollPane(SpeedListButton,new ScrollPane.ScrollPaneStyle());
        ...
        Label NowTime=new Label("  当前时间:  "+Float.toString(build.NowTime),new Label.LabelStyle(build.Bit,build.Bit.getColor()));
        ...
        Add.addListener(new InputListener(){
                ...
                public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                    ...
                    group.remove();
                    new BuildSetActions(build,null);
        ...
        Remove.addListener(new InputListener(){
                ...
                public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                    if(checked!=null){
                    SpeedListButton.clearChildren();
                    list.remove(checked.getUserObject());
                    BuildList(SpeedListButton);
                    }
                    return true;
                }
            });
    ...
    public void BuildList(Table table){
        for(int i=0;i<list.size();i++){
            Label speed=new Label(" 开始时间: "+Float.toString(list.get(i).startTime)+" 开始值: "+Float.toString(list.get(i).startValue)
                                  +"\n"+" 结束时间: "+Float.toString(list.get(i).endTime)+" 结束值: "+Float.toString(list.get(i).endValue)+"\n"+" 非线性种类: "+Float.toString(list.get(i).easingType),new Label.LabelStyle(build.Bit,build.Bit.getColor()));
            //bpm.setFontScale(back.getHeight()/16/build.BpmNumber.getHeight());
            speed.setWidth(back.getWidth());
            speed.setUserObject(list.get(i));
            AddListen(speed);
            table.add(speed).padTop(20);
            table.row();
